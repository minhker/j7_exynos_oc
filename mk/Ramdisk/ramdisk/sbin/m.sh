...
PATH=/sbin:/system/sbin:/system/bin:/system/xbin:/ms
export PATH
RUN=/sbin/busybox;
LOGFILE=/data/ms/boot.log
REBOOTLOGFILE=/data/ms/reboot.log
...
if [ ! -e /data/ms ]; then
	mkdir -p /data/ms
	chown -R root.root /data/ms
	chmod -R 755 /data/ms
fi

if [ ! -e /data/msLogcat ]; then
  mkdir -p /data/msLogcat
  chown -R root.root /data/msLogcat
  chmod -R 755 /data/msLogcat
fi

   log_print "Backup previous logcat"

if [ -e /data/ms/Refined_logger.log ]; then
  cp "/data/ms/Refined_logger.log" "/data/msLogcat/Refined_Logger_$(date +"%d-%m-%Y %H:%M:%S").log"
fi

for FILE in /data/ms/*; do
	$RUN rm -f $FILE
done;
...
log_print "------------------------------------------------------"
log_print "**ms boot script started at $( date +"%d-%m-%Y %H:%M:%S" )**"
...
# RefinedLogger
/system/bin/logcat *:E > /data/ms/Refined_logger.log
...
mount -o remount,rw /cache
   ...
   log_print "**ms early boot script finished at $( date +"%d-%m-%Y %H:%M:%S" )**"
   log_print "------------------------------------------------------"
